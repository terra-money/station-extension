...
-
- **`progressTracker`** - `boolean` - Display a `<ProgressTracker />` in the activity item.
...
Displays a `<ProgressTracker />` in the activity item.
<div className="story_with_source_below">
  <Canvas of={ActivityListItemStories.WithProgressTracker} />
  <Source code={`
    <ActivityListItem
  variant={"loading"}
  chain={{ icon: "https://station-assets.terra.dev/img/chains/Terra.svg", label: "Terra" }}
