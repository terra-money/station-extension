...
import { getChainIDFromAddress, validateMsg } from 'utils';
...
const lessThan = (max: number, label = 'Amount', optional = false) => {
    return (value = 0) => {
        ...
        return (value > 0 && value <= max) || `${label} must be between 0 and ${max}`;
    };
};
...
const input = (max: number, decimals = 6, label = 'Amount', optional = false) => {
    if (max <= 0) {
        return always('Insufficient balance');
    }
    return {
...
const recipient = () => {
    return {
        ...
        validate: (recipient = '') => validateRecipient(recipient) || 'Invalid recipient',
    };
};
...
const memo = () => (value?: string) => {
    ...
    return ['<', '>'].every(char => !value.includes(char)) || 'Memo cannot include `<` or `>`';
};
...
/* tns */
export const validateRecipient = (address: string) => AccAddress.validate(address) || address.endsWith('.ust');
