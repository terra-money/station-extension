...
export const useLocalNodeInfo = (chainID: string) => {
    ...
    return useQuery(
        ...
        async () => {
            const { data } = await axios.get('cosmos/base/tendermint/v1beta1/node_info', {
...
export const useValidateLCD = (lcd?: string, chainID?: string, enabled?: boolean) => {
    return useQuery(
        [lcd, chainID],
        async () => {
            if (!lcd || !chainID) {
        ...
        networks.map(({ chainID, prefix, lcd }) => {
            return {
                ...
                queryFn: async () => {
                    ...
                    const { data } = await axios.get(`/cosmos/bank/v1beta1/balances/${randomAddress(prefix)}`, {
