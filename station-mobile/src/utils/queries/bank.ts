...
import { getChainIDFromAddress } from 'utils';
...
export const [useBankBalance, BankBalanceProvider] = createContext<CoinBalance[]>('useBankBalance');

export const useInitialBankBalance = () => {
    const lcd = useInterchainLCDClient();
    console.log('lcd bank', lcd.bank);
    const addresses = useInterchainAddresses();
    console.log('useInitialBankBalance-addresses', addresses);

    return useQueries(
        Object.entries(addresses ?? {}).map(([chainID, address]) => {
            console.log('chain id, adress', chainID, address);
            return {
...
export const useBalances = () => {
    ...
    return useQuery(
        ...
        async () => {
            ...
            const balances = await Promise.all(chains.map(chain => lcd.bank.balance(addresses[chain])));
