...
import { CURRENCY_KEY, STATION_ASSETS, ASSETS } from '../../constants';
...
const AXELAR_TOKENS: Record<string, string> = {
    'ibc/B3504E092456BA618CC28AC671A71FB08C6CA0FD0BE7C8A5B5A3E2DD933CC9E4': 'uusdc',
    'ibc/CBF67A2BCF6CAE343FDF251E510C8E18C361FC02B23430C121116E0811835DEF': 'uusdt',
};
...
export const useExchangeRates = () => {
    ...
    return useQuery(
        ...
        async () => {
            const [{ data: TFM_IDs }, { data: prices }, fiatPrice] = await Promise.all([
                axios.get<Record<string, string>>('station/tfm.json', {
                    baseURL: ASSETS,
                }),
                axios.get<Record<string, TFMPrice>>('https://price.api.tfm.com/tokens/?limit=1500'),
                (async () => {
                    if (currency.id === 'USD') {
                        return 1;
                    }
