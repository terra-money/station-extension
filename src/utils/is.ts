const isLocal = (wallet?: Wallet | LegacySingleWallet): wallet is LocalWallet => {
    if (!wallet) {
        return false;
    }
...
const isMultisig = (wallet?: Wallet | LegacySingleWallet): wallet is MultisigWallet => {
    if (!isLocal(wallet)) {
        return false;
    }
...
const isLedger = (wallet?: Wallet | LegacySingleWallet): wallet is LedgerWallet => {
    if (!wallet) {
        return false;
    }
